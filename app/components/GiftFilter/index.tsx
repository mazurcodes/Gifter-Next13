...
const GiftFilter = ({ statusFn, data }: GiftFilterProps) => {
  ...
  return (
    <div className="filter-bar my-4 text-sm font-semibold text-gray-500">
      <button
        className={`filter-button border rounded-lg w-24 mr-2 ${filter === '' && 'border-4 border-dashed border-l-orange-400 border-b-blue-400 border-r-red-500 border-t-green-500 text-gray-700'}`}
        onClick={() => setFilter('')}
      >
        All
      </button>
